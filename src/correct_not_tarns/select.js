...
export default function($ori, $trans){
  ...
  function set_trans($trans, $target){
    var select = '';
    ...
    var $selected = $trans.contents().find(select)
    if ($selected.length === 1) {
      ...
      alert('未找到匹配项，请手动匹配');
      $trans.contents().one('click', function(e){
